...
import { 
  ...
  MapPin, DollarSign, Calendar, Briefcase, Sparkles 
} from 'lucide-react';
...
export default function PostJob() {
  ...
  const handleSubmit = async (e) => {
    ...
    if (description.length < 50) {
      setError('Description is too brief. Must be at least 50 characters.');
      setLoading(false);
      return;
    }

    // Build API payload matching CreateJobSchema
    const payload = {
      title,
      description,
      jobType,
      isRemote,
      location: isRemote ? '' : location || undefined,
      skillsRequired,
      ...
      <div className="glass-panel p-6 sm:p-8 rounded-3xl border shadow-sm flex flex-col gap-6">
        <div className="text-center flex flex-col gap-1 border-b border-slate-200/50 dark:border-slate-800/20 pb-4">
          ...
          <p className="text-xs text-slate-500 dark:text-slate-400">
            Publish a vacancy, configure credentials requirements, and let the AI score candidate resumes automatically.
          </p>
        </div>
        ...
        <form onSubmit={handleSubmit} className="flex flex-col gap-5">
          ...
          <div className="flex flex-col gap-1.5">
            <label htmlFor="jobTitle" className="text-xs font-bold text-slate-650 dark:text-slate-400 uppercase tracking-wider">
              Job Title
            </label>
            <input
              id="jobTitle"
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            {/* Job Type select */}
            <div className="flex flex-col gap-1.5">
              <label htmlFor="jobType" className="text-xs font-bold text-slate-650 dark:text-slate-400 uppercase tracking-wider">
                Employment Type
              </label>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            ...
            <div className="flex flex-col gap-2 justify-center p-3 bg-white/30 dark:bg-darkbg-100/10 border border-slate-200/50 dark:border-slate-800/20 rounded-xl">
              ...
              <label className="flex items-center gap-2 cursor-pointer select-none text-xs font-semibold text-slate-700 dark:text-slate-350">
                <input
                  type="checkbox"
                  checked={isRemote}
          ...
          <div className="flex flex-col gap-2 border-t border-slate-200/50 dark:border-slate-800/20 pt-4">
            ...
            <div className="flex flex-wrap gap-1.5 mt-2 bg-slate-50 dark:bg-darkbg-100/10 p-3.5 rounded-2xl border border-slate-200/50 dark:border-slate-800/20">
              {skillsRequired.length === 0 ? (
                <span className="text-[11px] text-slate-400 italic">No requirement tags set. Add some above so AI can calculate matches.</span>
              ) : (
          ...
          <div className="flex flex-col gap-1.5 border-t border-slate-200/50 dark:border-slate-800/20 pt-4">
            ...
            <textarea
              ...
              rows={8}
              required
              placeholder="Outline specific responsibilities, required years of experience, workflow details, daily tasks, benefit packages, and company values..."
              value={description}
